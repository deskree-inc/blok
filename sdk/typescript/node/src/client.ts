...
export class BlokClient {
	...
	constructor(config: BlokSDKConfig) {
	...
	private async callWorkflow<T = unknown>(
		...
		const base64Workflow = encodeBase64(JSON.stringify({ request, workflow }));
...
export function createBlokClient(config: BlokSDKConfig): BlokClient {
	return new BlokClient(config);
}
